...
#' # Train Model
xgtrain <- xgb.DMatrix(data = as.matrix(train[, xgb_features]),
...
#' # Plot Importance
importance <- xgb.importance(model = model, feature_names = xgb_features)
xgb.plot.importance(importance)

#' # Generate Predictions
predictions <- predict(model, as.matrix(test[, xgb_features])) %>%
